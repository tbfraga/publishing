...
Then, the equation can be used for calculation of multicriteria classification weights of each product, $\rho_{p}$:

\begin{equation}
	\rho_{p} = (wb + wlt) * \frac{b_p}{lt_p} + \sum{w_{\sigma_q}*\frac{b_{e_p}}{lt_{e_p}}} + \sum{w_{\beta_b}*w_{p\beta_b}}
\end{equation}
...
$\textrm{B}$ is the set of binary criteria, \quad $\textrm{B} = \{\beta_b\}_{b=NQ+1}^{NQ+NB}$;

$p$ is the index used to represent each product, \quad $p= 1,...,\textrm{NP}$;

$e_p$ is the lead time billing equivalent to product $p$;
