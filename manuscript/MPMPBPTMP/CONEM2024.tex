...
Conforme é possível verificar na Tab. 4, considerando $T_1 = 55$, teríamos um excesso de estoque para o segundo período. Isso conmprova que a determinação do tempo máximo de produção do lote no primeiro período deve levar em conta as informações relativas aos outros períodos. Outra questão relevante a ser considerada é o melhor momento para produção. Isso é válido para todos os períodos considerados. É importante aqui também ressaltar que estoque implica em custos, portanto é sempre melhor produzir no momento mais tarde possível. 

A seção à seguir apresenta uma adaptação do modelo matemático proposto por Fraga(2024), para solução do DMPMPpBPTM.
