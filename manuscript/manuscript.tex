...
\item \textbf{others ($\rho$)}: limited capacity units (LCU), storage capacity by product (PSC), storage capacity (SC), work-in-process storage (WIP), release time (RT), due dates (DD), incompatible families (IF), different processing rates (DPR).
\end{enumerate}

So, to describe a speciﬁc multi-product batch problem, we propose the notation $\sigma|\lambda|\mu|\pi|\eta|\gamma|\iota|\delta|\rho$. The multi-product batch problems and solution approaches discussed in the references cited in this paper are summarized in Tab. \ref{tab:review}.
...
\cline {1-3} \\
Ravemark and & $MS|Pl|SPC|D|Cc|$ & convex mixed-integer\\
Rippin (1998) & $St|Dt|C|WIP$ & linear programming \\
& & model solved with \\
& & DICOPT++ \\
\cline {1-3} \\
...
\cline {1-3} \\
\cite{MendezEtAll2000} & $SS|\lambda|p,s\textrm{-}batch|P,Sc|\eta|\gamma|$ & two-step systematic \\
& $Dt|WIPC|RT, DD$& methodology \\
\cline {1-3} \\
\cite{HeHui2008} & $SS|Pl|MPC|Sc|SP|$ & rule-based genetic \\
& $St|Dt|C_{max}, TT|LCU, DD$ & algorithm \\
\cline {1-3} \\
\cite{FumeroEtAl2016} & $MS|Pl|MPC|D,P,Sc|Cc|$ & mixed-integer linear  \\
& $St|Dt|NP|LCU,USC$ & programming model \\
& & solved with CPLEX \\
\cline {1-3} \\
...
& & and exact algorithm \\
\cline {1-3} \\
\cite{LiEtAl2022} & $SS|Pl|p\textrm{-}batch|Sc,B|SP|$ & mixed-
integer linear \\
& $St|Dt|C_{max}|LCU, RT, IF$ & programming and \\
 & & a constraint-guided \\
 & & artificial immune \\
