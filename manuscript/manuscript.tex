...
\begin{abstract} 
The multi-product batch (MPB) problems have been widely studied by the researchers. Among the most complex MPB problems are the multi-product p-batch scheduling (MPPBS) problems, where jobs must be scheduled on parallel batch processing machines. When applying some local search heuristics to solve a MPPBS problem, it can be necessary to determine, in each iteration, the maximum processing time for batches of compound jobs considering that the jobs that make up the batches have different processing rates, and that there are limitations for the amount produced for each product, as well as for the set of products of the compound  batch. Using the denominations present in the literature, this sub-problem can be simply referred as a single stage multi-product p-batch sizing (SSMPPBS) problem . Once a SSMPPBS problem needs to be solved in each iteration, the efficiency of the method developed for solving a SSMPPBS problem has a huge impact on the computational cost of the local search algorithm. In addition, for some industries, efficiently solving an SSMPPBS problem can have a significant impact on storage costs and customer satisfaction. In this paper we presente a mathematical model and an exact method with polynomial time complexity for the SSMPPBS problem. We developed a LINGO solver for solving the mathematic model, and a C ++ solver for application of the proposed exact method. We tested both solvers, comparing the results for a series of benchmarks with sizes varying between small and very large. As result, we find that the exact method can obtain optimal solutions in a very short time (just few seconds), even when solving very large instances. 
\end{abstract}
%%Graphical abstract
...
%%Research highlights
\begin{highlights}
\item taxonomy and notation for the multi-product batch problems;
\item multi-product p-batch processing time maximization (MBPTM) problem definition;
...
\item \textbf{production mode ($\mu$)}: single-product campaing (SPC), mixed-product campaign (MPC), serial batching (s-batch) and/or parallel batching (p-batch);
\item \textbf{problem:($\pi$)}  : design (D), planning (P), sizing (Sz), scheduling (Sc) and/or balancing (B); 
\item \textbf{time horizon ($\eta$)}: single period (SP) or multiperiod (MP), cyclic (Cc);
\item \textbf{scenario ($\gamma$)}: static (St) or dynamic (Dn);
\item \textbf{input ($\iota$)}: deterministic (Dt) and/or stochastic (St);
\item \textbf{performance measure ($\delta$)}: capital cost (C), setup cost (SC), holding cost (HC), work-in-process cost (WIPC), net profit (NP), makespan (C$_{\textrm{max}}$), total completion time ($TC$), weighted completion time ($TWC$), maximum lateness ($L_{max}$), maximal tardiness ($T_{max}$), total tardiness (TT), the total weighted earliness (TWE), total weighted tardiness (TWT), the number of tardy jobs ($NTJ$), weighted number of tardy jobs ($WNTJ$), earliness/tardiness ($E/T$), weighted $E/T$ ($wE/T$), and processing time (PT);
\item \textbf{others ($\rho$)}: limited capacity units (LCU), storage capacity by product (PSC), storage capacity (SC), work-in-process storage (WIP), release time (RT), due dates (DD), incompatible families (IF), different processing rates (DPR), sequence-dependent setup times (SDST).
\end{enumerate}
...
Cerdá (2003) &$TWE, TWT|RT, DD, SDST$ & a rescheduling algorithm\\
\cline {1-3} \\
...
When we began to propose an algorithm to solve the studied problem through a local search method, we identified the need to determine, at each iteration, a maximum processing time for predefined multi-product \emph{p-batches}, based on known restrictions, so that the new solution built was a feasible solution and at the same time with the best fitness. This problem can be classified as a multi-product $SS|SU|p\textrm{-}batch|Sz|SP|St|Dt|PT|LCU, PSC, SC, DPR$ batch problem. For simplification purposes, we named this problem multi-product p-batch processing time maximization (MBPTM) problem. 
