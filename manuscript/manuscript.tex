...
\item \textbf{layout($\lambda$)}: single unit (SU), flowshop (Fl), parallel (Pl), or networked (Nt) (convergent (CN) and /or divergent (DN));
...
\item \textbf{planning horizon ($\eta$)}: single period (SP) or multiperiod (MP);
\item \textbf{scenario ($\gamma$)}: static (St) or dynamic (Dn);
\item \textbf{input ($\iota$)}: deterministic (Dt) and/or stochastic (St);
\item \textbf{performance measure ($\delta$)}: capital cost ($C$), makespan ($C_{max}$), total completion time ($TC$), weighted completion time ($TWC$), maximum lateness ($L_{max}$), maximal tardiness ($T_{max}$), total tardiness ($TT$), the total weighted tardiness ($TWT$), the number of tardy jobs ($NTJ$), weighted number of tardy jobs ($WNTJ$), earliness/tardiness ($E/T$) and/or weighted $E/T$ ($wE/T$);
\item \textbf{restrictions ($\rho$)}: incompatible jobs (IJ).
\end{enumerate}

So, to describe a speciﬁc multi-product batch problem, we propose the notation $\sigma|\lambda|\mu|\pi|\eta|\gamma|\iota|\delta|\rho$. The multi-product batch problems and solution approaches discussed in the references cited in this paper are summarized in Tab. . 
